...
Extract this ZIP.
Open `config.js`.
Replace `PASTE_YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE` with your deployed Apps Script URL ending in `/exec`.
Upload all files to the root of your GitHub Pages repository.
Commit the changes and wait for GitHub Pages to publish.
Open the site with `?v=23` appended once, then use Ctrl+Shift+R to bypass older cached files.
Included files
